...
public class MayinTarlasi implements MouseListener {
	...
	public void mouseClicked(MouseEvent e) {
		...
		if(e.getButton()==1) {
			System.out.println("sol tiklandi");
			if(b.isMine()) {
				JOptionPane.showMessageDialog(frame, "Mayina Bastiniz, Oyun Bitti!");
				print();
			}else {
				open(b.getRow(),b.getCol());
				if(openButton==(board.length*board[0].length)-10) {
					JOptionPane.showMessageDialog(frame, "Tebrikler Oyunu Kazandiz!");
					print();
				}
			}
		}else if(e.getButton()==3){
			System.out.println("sag tiklandi");
			if(!b.isFlag()) {
